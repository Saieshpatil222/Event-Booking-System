...
public class BookingControllerTest {
    ...
    public void createBookingTest() throws Exception {
        ...
        mockMvc.perform(post("/booking/{userId}/{eventId}/{promoCodeId}", userId, eventId, promoCode).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(bookingDto)))//writeValueAsString(bookingDto) converts the bookingDto Java object into a JSON string.
                .andExpect(status().isOk()).andExpect(jsonPath("$.userId").value(savedBooking.getUserId())).andExpect(jsonPath("$.eventName").value(savedBooking.getEventName())).andDo(print());
    ...
    public void createBookingWithoutPromoCodeTest() throws Exception {
        ...
        mockMvc.perform(post("/booking/{userId}/{eventId}", userId, eventId).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(bookingDto))).andExpect(status().isOk()).andExpect(jsonPath("$.userId").value(savedBooking.getUserId())).andExpect(jsonPath("$.eventId").value(savedBooking.getEventId())).andExpect(jsonPath("$.eventName").value(savedBooking.getEventName())).andExpect(jsonPath("$.bookingId").value(savedBooking.getBookingId())).andDo(print());
    ...
    public void getSingleBookingTest() throws Exception {
        ...
        mockMvc.perform(get("/booking/{bookingId}", bookingId).contentType(MediaType.APPLICATION_JSON).with(user("normal").roles("NORMAL")).content(objectMapper.writeValueAsString(bookingDto))).andExpect(status().isOk()).andExpect(jsonPath("$.userId").value(bookingDto.getUserId())).andExpect(jsonPath("$.eventId").value(bookingDto.getEventId())).andExpect(jsonPath("$.eventName").value(bookingDto.getEventName())).andExpect(jsonPath("$.bookingId").value(bookingDto.getBookingId())).andDo(print());
...
//    }
    ...
    public void deleteBookingTest() throws Exception {
        ...
        mockMvc.perform(delete("/booking/{bookingId}", bookingId).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(bookingDto1))).andExpect(status().isOk());
